...
func TestMlxlinkExporterService_ExecStartUsesBuiltInConfiguration(t *testing.T) {
	service, err := os.ReadFile("mlxlink_exporter.service")
	if err != nil {
		t.Fatalf("read systemd service: %v", err)
	}

	const want = "ExecStart=/usr/local/bin/mlxlink_exporter"
