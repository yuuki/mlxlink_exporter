...
This document describes `mlxlink_exporter`.

## 1. Background and Goals

The physical layer below the kernel's sysfs view — bit error ratios, per-lane raw errors, FEC histograms, SerDes tuning and transceiver diagnostics — lives in the adapter firmware and is only reachable through NVIDIA's MFT tooling, in practice `mlxlink`.
...
- **An isolated process.** `mlxlink` may need elevated privileges, and firmware access can hang. Keeping the polling loop out of the Prometheus scrape path contains that blast radius.
...
4. After each device the poller publishes a fresh set keyed by device name: the devices this sweep has already visited, plus the previous entries of the devices it has not reached yet. A scrape that lands mid-sweep therefore sees fresh data for the devices already visited and the previous data for the rest, never a gap. A device that discovery no longer reports is absent from the first publish of the sweep onwards.
...
- **Bounded output.** stdout is capped at 4 MiB and stderr at 4 KiB; bytes past the cap are dropped instead of buffered. The run then fails from the buffer state rather than from the exit status: a binary that overproduces and then exits successfully must not have its truncated output accepted. Overproduction does not stop the process early, so a binary that never stops writing is ended by the invocation timeout and still reported as `output_too_large`, because the buffer state is examined before the exit error is classified.
- **Bounded lifetime.** Each invocation runs under `exec.CommandContext` with a `--command-timeout` deadline, so the deadline kills the direct child. A grandchild that inherited stdout would still hold the pipe open and keep `Wait` blocked, so `Cmd.WaitDelay`, set to the same timeout, closes the pipes; that is what bounds the caller in this case.
...
- **Base-field spellings are centralised.** The MFT key for each module, operational and physical-counter field lives in `fieldAliases`, as do section names: one observed MFT 4.34.1 spelling per canonical field. The FEC and SerDes parsers own their internal structural keys, lane headers and allowlisted parameter names because those sections require schema-level validation rather than scalar field lookup.
...
- **Missing is not zero.** `N/A`, an empty string and anything unparsable yield an invalid value, which the collector omits. Only the bracketed range suffix is removed before parsing; what remains must be a number in full, so a partially readable value is dropped rather than mined for its leading digits, and a literal too large for float64 is dropped along with the malformed ones, because `strconv.ParseFloat` reports it out of range and the ±Inf it saturates to is not the number that was measured. A literal too small is the opposite case: `ParseFloat` reports no error and returns a correctly rounded 0, which is kept, because 0 is what the reading is worth at that magnitude. Flag families are stricter: they accept only `0`/`1` and are taken all-or-nothing, because a partially decoded family would renumber its lanes relative to its neighbours. `DataPath state` compares exactly against `DPActivated`, so an unrecognised state reads as inactive and an absent one voids the family.
...
The fallback carries nothing between sweeps: it is decided per device on every sweep, so a query that keeps exiting non-zero is retried every time. On an MFT release where `--show_eye` always fails, each device costs two invocations per sweep — the Eye query, then the normal combined one — instead of one; if the combined extras (`--rx_fec_histogram --show_histogram --show_serdes_tx`) are rejected as well, it costs three, ending at baseline. The sweep duration grows in proportion and must still fit inside the poll interval.

That state is visible as `mlxlink_collection_errors_total{reason="exit_error"}` rising on every sweep while `mlxlink_collector_up` stays 1, and the operational answer is to turn the Eye flag off: the exporter is already publishing the fallback data, so the extra invocation buys nothing. The same reasoning applies without Eye, where a persistently rejected combined query costs two invocations per device per sweep.
...
If a tick fires while a sweep is running, that tick is dropped and `mlxlink_sweep_overlaps_total` is incremented once. Dropping the tick is the point: a slow sweep must not chain into back-to-back `mlxlink` invocations against the firmware. The counter is process-wide and carries no labels — the condition it reports belongs to the sweep, not to any one device. Exactly one pending tick is consumed per sweep: Go tickers coalesce the ticks they missed, so no more than one can be pending however long a sweep runs, and taking one keeps the drain finite. The channel length cannot be used to size that drain — since Go 1.23 a ticker channel reports a length of 0 while a tick is pending.
...
Compatibility note: overlaps used to be recorded per device as `mlxlink_collection_errors_total{reason="overlapping"}`, and as the same reason on `mlxlink_pcie_eye_collection_errors_total`. That reason no longer exists; `mlxlink_sweep_overlaps_total` replaces it. Alerts and dashboards that match `reason="overlapping"` must be moved to the new counter.
...
| `output_too_large` | runner | stdout passed 4 MiB, so the excess was dropped and the response rejected |
...
A dropped tick is not part of this taxonomy. It is a property of the sweep rather than of a device, so it is counted by the unlabeled `mlxlink_sweep_overlaps_total` (§5.4) and never appears in either error counter.
...
- **Grandchild processes**: the timeout kills the direct child only, and nothing signals its descendants. A grandchild that inherited stdout is therefore left running, and the wait delay closing the pipes is what still bounds the caller. `mlxlink` is not known to fork such children — this is a bound on the failure mode, not an observed one.
- **Overlap accounting is approximate both ways**: ticker coalescing makes one overrun sweep a single `mlxlink_sweep_overlaps_total` increment regardless of how many intervals it spanned, and a tick landing exactly during the drain is counted as an overlap that never happened (§5.4). The metric detects "the interval is too short", not "how many sweeps were lost".
- **A rejected optional query is paid for on every sweep**: fallback is not remembered between sweeps, so an `--show_eye` that always exits non-zero doubles the invocations per device indefinitely, or triples them if the combined extras fail too (§5.4). Detecting the unsupported flag once and disabling it would need a rule for when to re-test it, so the retry is kept and the decision is left to the operator.
...
- Track MFT spellings as new releases are captured: base fields and section names in `fieldAliases`, structural keys and parameter names in the dedicated FEC, SerDes and Eye parsers, with a real fixture for each new output shape.
