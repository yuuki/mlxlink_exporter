...
The fallback carries nothing between sweeps: it is decided per device on every sweep, so a query that keeps exiting non-zero is retried every time. On an MFT release where `--show_eye` always fails, each device costs two invocations per sweep — the Eye query, then the normal combined one — instead of one; if the combined extras (`--rx_fec_histogram --show_histogram --show_serdes_tx`) are rejected as well, it costs three, ending at baseline. The sweep duration grows in proportion and must still fit inside the poll interval. That state is visible as `mlxlink_collection_errors_total{reason="exit_error"}` rising on every sweep while `mlxlink_collector_up` stays 1, and the operational answer is to turn the Eye flag off: the exporter is already publishing the fallback data, so the extra invocation buys nothing. The same reasoning applies without Eye, where a persistently rejected combined query costs two invocations per device per sweep.
